...
function validateRecipe (recipe: EditRecipe, duration: number): string | null {
  const validations: Array<[boolean, string]> = [
    ...
    [
      ...
      `Trim end time cannot exceed the video duration (${Math.floor(
        duration
      )}s).`
    ],
    [
...
export function useVideoEditor() {
  ...
  const updateRecipe = useCallback(
    (patch: Partial<EditRecipe>) => {
      setHistory(prevHistory => {
        const current =
          prevHistory[historyIndex] ?? prevHistory[0] ?? createInitialRecipe()
        const next: EditRecipe = { ...current, ...patch }
        if (next.format === 'gif') {
        ...
        if (currentString === nextString) return prevHistory

        const newHistory = [...prevHistory.slice(0, historyIndex + 1), next]
  ...
  const isValidValue = (key: keyof EditRecipe, val: any): boolean => {
    switch (key) {
      ...
      case 'customWidth':
        return (
          typeof val === 'number' && !isNaN(val) && val >= 16 && val <= 7680
        )
      case 'customHeight':
        return (
          typeof val === 'number' && !isNaN(val) && val >= 16 && val <= 7680
      ...
      case 'trimEnd':
        return (
          val === null || (typeof val === 'number' && !isNaN(val) && val >= 0)
        )
      case 'rotate':
    ...
    try {
      ...
      if (hasRecipeParams) {
        ...
        if (Object.keys(updatedPatch).length > 0) {
          setHistory(prevHistory => {
            const baseRecipe = prevHistory[0] ?? createInitialRecipe()
            const next: EditRecipe = { ...baseRecipe, ...updatedPatch }
            if (next.format === 'gif') {
              next.keepAudio = false
            }
            return [next]
        ...
        try {
          ...
          if (raw) {
            ...
            if (isValidRecipe(parsed)) {
              setHistory([parsed as EditRecipe])
              setHistoryIndex(0)
              return
            }
          }
        ...
        if (saved) {
          ...
          setHistory(prevHistory => {
            const baseRecipe = prevHistory[0] ?? createInitialRecipe()
            const next: EditRecipe = {
              ...baseRecipe,
              preset: parsed.preset ?? baseRecipe.preset,
              quality: parsed.quality ?? baseRecipe.quality,
              speed: parsed.speed ?? baseRecipe.speed,
              customWidth: sanitizeDimension(
                parsed.customWidth,
                baseRecipe.customWidth
              ),
              customHeight: sanitizeDimension(
                parsed.customHeight,
                baseRecipe.customHeight
              )
            }
            return [next]
          })
  ...
  const handleFileSelect = useCallback(
    async (selectedFile: File) => {
      ...
      try {
        ...
        const shouldApplySuggestion = recipe.preset === DEFAULT_RECIPE.preset
  ...
  const handleExport = useCallback(async () => {
    ...
    try {
      ...
      if (err instanceof FFmpegLoadError) {
        ...
        setError(
          'This video format is not supported. Try converting to MP4 first.'
        )
      } else {
  ...
  }, [file, recipe.keepAudio, updateRecipe])
